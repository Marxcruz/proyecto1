...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on('appointment_status_updated', (data) => {
      console.log('Evento de actualización de cita recibido:', data);
    ...
    return () => {
      socket.disconnect();
      console.log('Desconectado del socket en dashboard de administrador');
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchDoctors = async () => {
      try {
        ...
        // No mostraremos un toast de error aquí para no ser intrusivos si solo falla esta parte
        // toast.error(error.response?.data?.message || 'Error al cargar los doctores');
      }
    };
      ...
      setAppointments((prevAppointments) =>
        prevAppointments.map((appointment) =>
          appointment._id === appointmentId
            ? { ...appointment, estado: status } // Asegurarse de que el nuevo estado se asigna
            : appointment
        )
      );
  ...
  // eliminar cita por el administrador
